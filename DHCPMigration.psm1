    ...
    Process {
        ...
        if ($PSBoundParameters.ContainsKey("ClassName")) {
            ...
            for ($n = 0; $n -lt $ClassName.Length; $n++) {
                ...
                foreach ($fromCls in $allFromClasses) {
        ...
        if ($differences.Count -gt 0) {
            foreach ($def in $differences) {
                ...
                $optArgs = @{
                    ...
                    Confirm      = switch ($PSBoundParameters.ContainsKey("Confirm")) {
                        $true {
...
Function PrepareConnections([string]$SetName, [System.Collections.IDictionary]$BoundParameters) {
    ...
    if ($SetName -like "*Credential") {
        if ($BoundParameters.ContainsKey("ToCredential")) {
            ...
            $private:toHash.ComputerName = switch ($BoundParameters.ContainsKey("ToServer")) {
                $true {
                    $BoundParameters["ToServer"]
                } $false {
                    $env:COMPUTERNAME
                }
            }
